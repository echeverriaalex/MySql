drop database if exists Guia3_GROUPBY;
create database if not exists Guia3_GROUPBY;
use Guia3_GROUPBY;
...
select * from empleado order by apellido1 asc;
...
select * from empleado order by codigo_departamento asc;
...
select * from empleado;
select concat(nombre, " ", ifnull(apellido1, " ") , " " , ifnull(apellido2, " ")) 
	as "Nombre completo" from empleado;
...
select * from empleado where apellido2 like "Lopez";
